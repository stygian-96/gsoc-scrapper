...
const getArrayofTitle = async() => {
    const list = []
    let fileData = await fsp.readFile(path.resolve(__dirname, 'tryJson.json'))
    ...
    listYears.forEach(listYear => {
        ...
        for(var i=0 ;i<len;i++){
            ...
            if(ob[titleTrim] != null){
                obj[`${listYear}`][i][`noOfTimesAfter2k16`] = ob[titleTrim].count
                obj[`${listYear}`][i][`yrListAfter2k16`] = ob[titleTrim].yrList
            }
        }
    })
    console.log(obj)
    await fsp.writeFile(path.resolve(__dirname, 'tryJson.json'), JSON.stringify(obj));
}
